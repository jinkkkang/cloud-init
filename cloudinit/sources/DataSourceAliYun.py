...
class DataSourceAliYun(sources.DataSource):
    ...
    def crawl_metadata(self):
        ...
        skip_cb = None
        exe_cb_whole_meta = self._skip_whole_meta_path_aliyun_cb
        try:
            ...
            try:
                ...
                util.logexc(
                    LOG,
                    "Failed reading from instance metadata address %s, try fall back to directory tree style",
                    self.metadata_address,
                )
    ...
    def _skip_or_refresh_stale_aliyun_token_cb(self, msg, exception):
        """Callback will not retry on SKIP_USERDATA_VENDORDATA_CODES or if no token
        is available."""
    ...
    def _skip_whole_meta_path_aliyun_cb(self, msg, exception):
    ...
    def _get_data(self):
        ...
        self.vendordata_raw = self._crawled_metadata.get("vendor-data", None)
        return True
